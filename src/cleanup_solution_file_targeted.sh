...
echo "Cleaning up solution file by removing only the consolidated API project references..."

# Create a backup of the original solution file
cp Ombi.sln Ombi.sln.backup
echo "Created backup: Ombi.sln.backup"

# Remove only the specific API projects that we consolidated, but keep Ombi.Api
echo "Removing consolidated API project references..."

# Remove each consolidated project entry and its corresponding build configurations
# We'll remove them one by one to be precise, but keep Ombi.Api
...
sed -i '' '/Ombi\.TheMovieDbApi/d' Ombi.sln

echo "Targeted solution file cleanup complete!"
echo "The Ombi.Api project (with IApi, Request, etc.) has been preserved."
echo "The Ombi.Api.External project remains as the consolidated API project."
echo "All other individual API projects have been removed."
