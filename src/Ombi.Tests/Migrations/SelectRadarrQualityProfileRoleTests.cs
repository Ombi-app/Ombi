...
namespace Ombi.Tests.Migrations
{
    [TestFixture]
    [NonParallelizable]
    public class SelectRadarrQualityProfileRoleTests
    {
        private const string MigrationId = "20260722000000_SelectRadarrQualityProfileRole";
        private const string AllowlistMigrationId = "20260723000000_UserSelectableQualityProfiles";
        private const string PreviousMigrationId = "20260417000001_AddPlexWatchlistUserStatus";

        [Test]
        public async Task SqliteMigration_CreatesRoleForFreshAndExistingInstall()
        {
            ...
            try
            {
                ...
                Assert.Multiple(() =>
                {
                    ...
                    Assert.That(context.Database.GetAppliedMigrations(), Does.Contain(AllowlistMigrationId));
