...
namespace Ombi.Tests
{
    ...
    public class PlexOAuthControllerTests
    {
        ...
        {
            // This endpoint is [AllowAnonymous] and overwrites the Plex configuration. Once setup is
            // complete (an admin exists) it must refuse, otherwise an unauthenticated caller could
            // plant their own Plex token and escalate to admin on login.
            SetAdminExists(true);

            var result = await _subject.OAuthWizardCallBack(1234);

            Assert.That(result, Is.InstanceOf<UnauthorizedResult>());
            // The trusted Plex configuration must not be read or overwritten.
