...
namespace Ombi.Notifications.Agents
{
    public class NtfyNotification : BaseNotification<NtfySettings>, INtfyNotification
    {
        ...
        {
            return settings.Enabled && !string.IsNullOrEmpty(settings.BaseUrl) && !string.IsNullOrEmpty(settings.Topic);
        }
        ...
        protected override async Task Send(NotificationMessage model, NtfySettings settings)
        {
            try
            {
                await Api.PushAsync(settings.BaseUrl, settings.Topic, settings.AuthorizationHeader, model.Subject, model.Message, settings.Priority);
            }
            catch (Exception e)
            {
